...
router.get('/createtable', async(req, res) => {
    let anfrage = `
        DROP TABLE IF EXISTS usersh; 
        CREATE TABLE usersh(id SERIAL PRIMARY KEY, username VARCHAR(255), 
        password VARCHAR(255), email VARCHAR(255), role VARCHAR(255));
        `;
    client.query(anfrage);
    
    res.send({ message: `table usersh in database ${process.env.PGDATABASE} created`});
});
...
router.post('/login', async(req, res) => {
    ...
    let check = await client.query('SELECT * FROM usersh  WHERE username = $1', [username])
    if (check.rowCount > 0) {
...
// create new user
router.post('/usersh', async(req, res) => {
    ...
    let checkmail = await client.query(`SELECT * FROM usersh WHERE email = $1;`, [email]);
    let checkname = await client.query(`SELECT * FROM usersh WHERE username = $1;`, [username]);
    if (checkmail.rowCount > 0) {
        ...
        const anfrage = `INSERT INTO usersh(username, password, email, role) VALUES($1, $2, $3, $4) RETURNING *;`;
        try {
...
// get all users
router.get('/usersh', async(req, res) => {
    const anfrage = `SELECT * FROM usersh;`;
...
// get one user via id
router.get('/usersh/id/:id', async(req, res) => {
    const query = `SELECT * FROM usersh WHERE id=$1;`;
...
// get user by name
router.get('/usersh/:username', async(req, res) => {
    const anfrage = `SELECT * FROM usersh WHERE username=$1;`;
...
// delete user by id
router.delete('/usersh/:id', async(req, res) => {
    const anfrage = `DELETE FROM usersh WHERE id=$1;`;
...
router.put('/usersh/:id', async(req, res) => {
    const anfrage = `SELECT * FROM usersh WHERE id=$1;`;
    ...
    if (result.rowCount > 0) {
        ...
        if (checkmails.rowCount > 1) {
            ...
            const updateanfrage = `UPDATE usersh SET
                username = $1,
                password = $2,
                email = $3,
                role = $4
